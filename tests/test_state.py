...
class TestSetDefault:
    ...
    def test_compression_reentry_safe(self, tmp_dir):
        ...
        run_state("init", "tmp/config.yaml", "headless=true")
        run_state("set-default", "tmp/config.yaml", "revise_cycle=0")
        run_state("set", "tmp/config.yaml", "revise_cycle=1")
        # Compression re-entry: init code runs again
        run_state("set-default", "tmp/config.yaml", "revise_cycle=0")
        out, _, _ = run_state("read", "tmp/config.yaml")
        assert "revise_cycle: 1" in out  # must NOT reset to 0


class TestWriteAndReadIds:
    def test_round_trip(self, tmp_dir):
        run_state("write-ids", "tmp/ids.txt", "RHAISTRAT-1001", "RHAISTRAT-1002")
        out, _, rc = run_state("read-ids", "tmp/ids.txt")
        assert rc == 0
        assert out.strip() == "RHAISTRAT-1001 RHAISTRAT-1002"

    def test_deduplicates(self, tmp_dir):
        run_state("write-ids", "tmp/ids.txt", "RHAISTRAT-1001", "RHAISTRAT-1002", "RHAISTRAT-1001")
...
class TestCleanRemoved:
    """Verify that the clean command no longer exists."""

    def test_clean_not_in_commands(self, tmp_dir):
        _, err, rc = run_state("clean")
        assert rc != 0
