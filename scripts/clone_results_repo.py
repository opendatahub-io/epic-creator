...
Only materializes the 'latest' symlink and decompose run report files.
Skips epic tasks, reviews, and HTML reports.
...
Examples:
    DATA_REPO_TOKEN=glpat-xxx python3 scripts/clone_results_repo.py \\
        redhat/rhel-ai/agentic-ci/epic-decomposer-results
    DATA_REPO_TOKEN=glpat-xxx python3 scripts/clone_results_repo.py \\
...
def main():
    ...
    # Materialize only the symlink and decompose run reports
    subprocess.run(
        [
            "git",
            "sparse-checkout",
